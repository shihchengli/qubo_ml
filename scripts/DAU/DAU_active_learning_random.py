...
# This workflow apllies the random selection approach that add random 50 reactions not in current training set into the new training set in the next iteration.
...
for fold in range(5):
    ...
    title = "fold_" + str(fold) + "_iter_" + str(iteration) + "_random_" + version
...
for fold in range(5):
    ...
    with open(predict_file, "w", newline="") as f:
        ...
        # Generate the next training file by random selection
        next_train_file = (
            dir
            + "/fold_"
            + str(fold)
        ...
        with open(next_train_file, "w", newline="") as f1:
            ...
            for random_id in random_list:
                if added == 50:
                    break
                if inputs[random_id][0] not in train_id:
                    train_writer.writerow(
                        [inputs[random_id][0], inputs[random_id][-1] / 10]
                        + inputs[random_id][1:-1]
                    )
                    added += 1
...
merge_metrics_file = dir + "/iter_" + str(iteration) + "_random_metrics_0.csv"
